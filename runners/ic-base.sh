...
## dataset region
DATASET=hpa2nd-kaggle     # HPA Single Cell Classification
TRAIN_CSV=$WORK_DIR/datasets/split/dino-kaggle.csv
DATA_DIR=$DATASETS_DIR/hpa-single-cell-image-classification/join_resize_inter_linear_train
...
train() {
  ...
  WANDB_RUN_GROUP="$DATASET-$ARCH-baseline" \
    $PY scripts/hpa/train_baseline.py \
    --device $DEVICE \
    --optimizer $OPTIMIZER \
