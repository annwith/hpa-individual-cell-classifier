#PBS -N hpa-priors
#PBS -q umagpu
#PBS -e experiments/logs/hpa-priors-fold.rs50-b32a1.r1.train.err
#PBS -o experiments/logs/hpa-priors-fold.rs50-b32a1.r1.train.log
...
#   WANDB_RESUME="$WANDB_RESUME" \
  WANDB_TAGS="$DATASET,$ARCH,lr:$LR,wd:$WD,ls:$LS,b:$BATCH,aug:$AUG,opt:$OPTIMIZER,sampler:$SAMPLER,fold:$FOLD_ID" \
...
# FOLD=0
# TAG=vanilla/$DATASET-$IMAGE_SIZE-${ARCH}-lr${LR}-b${BATCH}-ls$LS-$AUG-$OPTIMIZER-ema$EMA_STEPS-f$FOLD-$EID
# DEVICES=0,1 train_inference_fold &

# FOLD=1
# TAG=vanilla/$DATASET-$IMAGE_SIZE-${ARCH}-lr${LR}-b${BATCH}-ls$LS-$AUG-$OPTIMIZER-ema$EMA_STEPS-f$FOLD-$EID
# DEVICES=2,3 train_inference_fold &
# wait
